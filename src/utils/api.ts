...
export const KpiAPI = {
  ...
  updateObjmthvalue: async (data: {
    ...
    await axiosInstance.post('/V8670Kpi/update-objmthvalue', data);
  },

  updateObjmthmastervalue: async (data: {
    objid: string;
    objyear: number;
    objmth: number;
    objmthmastervalue: number;
    groupeid: string;
    comments?: string;
  }): Promise<void> => {
    await axiosInstance.post('/V8670Kpi/update-objmthmastervalue', data);
  },

  updateCumuls: async (): Promise<void> => {
    await axiosInstance.post('/V8670Kpi/update-cumuls');
  },

  insertStat: async (): Promise<void> => {
    await axiosInstance.get('/V8670Kpi/stat');
  },
